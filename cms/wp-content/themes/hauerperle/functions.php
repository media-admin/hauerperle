<?php
/**
 * Theme Funktionen und allgemeine Definitionen für die Website "hauerperle.at"
 */

/* --- General Theme Functions --- */
function hauerperle_theme_features()
{
...
add_action("initafter_setup_theme", "hauerperle_theme_features");

/* --- Activates Menu Support --- */
function hauerperle_register_menu()
{
  register_nav_menu( 'nav-header-main', 'Hauptnavigation', 'hauerperle' );
  register_nav_menu( 'nav-footer-main', 'Footernavigation', 'hauerperle' );
  register_nav_menu( 'nav-footer-menu', 'Footermenü', 'hauerperle' );
}

add_action("init", "hauerperle_register_menu");
...
function hauerperle_add_excerpts_to_pages()
{
  add_post_type_support("page", "excerpt");
}

add_action("init", "hauerperle_add_excerpts_to_pages");
...
function hauerperle_add_upload_ext($checked, $file, $filename, $mimes)
{
  if (!$checked["type"]) {
...
add_filter("wp_check_filetype_and_ext", "hauerperle_add_upload_ext", 10, 4);
...
function hauerperle_read_more_text($text, $post_id)
{
  return '<a class="more-link" href="' .
    get_permalink() .
    '">' .
    __("Read More", "hauerperle") .
    "</a>";
}

add_filter("the_content_more_link", "hauerperle_read_more_text", 10, 2);

/* --- Replaces the IP address at comments (IP-Anonymisierung lt. DSGVO) --- */
function hauerperle_replace_comment_ip()
{
  return "127.0.0.1";
}

add_filter("pre_comment_user_ip", "hauerperle_replace_comment_ip", 50);

/* --- Adding the Slug to body tag's class --- */
function hauerperle_add_slug_body_class($classes)
{
  global $post;
  if (isset($post)) {
...
add_filter("body_class", "hauerperle_add_slug_body_class");
...
function hauerperle_register_styles()
{
...
add_action("wp_enqueue_scripts", "hauerperle_register_styles");


function hauerperle_register_scripts() {
...
add_action( 'wp_enqueue_scripts', 'hauerperle_register_scripts' );
...
*/
  ...
  function hauerperle_get_backend_preview_thumb($post_ID)
  {
  ...
  function hauerperle_preview_thumb_column_head($defaults)
  {
  ...
  add_filter(
    ...
    "hauerperle_preview_thumb_column_head"
  );

  function hauerperle_preview_thumb_column($column_name, $post_ID)
  {
    if ($column_name == "featured_image") {
      $post_featured_image = hauerperle_get_backend_preview_thumb($post_ID);
  ...
  add_action(
    ...
    "hauerperle_preview_thumb_column",
    10,
    2
  );
